...
// Register
// FIXME: const “user“ to const “userData”
// FIXME: User.findOne({}, (err, result) => {}) to User.findOne({}, (err, “user”) => {})
...
router.post("/login", (req, res) => {
  ...
  User.findOne(
    { $or: [{ username: user.uniqueData }, { email: user.uniqueData }] },
    (err, result) => {
      if (result) {
...
// Get user
// Add playlists && musics && likes to userData
router.get("/user", (req, res) => {
  const uniqueData = req.query.uniqueData;

  User.findOne(
    { $or: [{ uuid: uniqueData }, { token: uniqueData }] },
    (err, user) => {
      if (user)
        res.send({
          uuid: user.uuid,
...
// After publish v1
// Login with google facebook twitter instagram spotyfy twitch
